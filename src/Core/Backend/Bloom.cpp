...
static Shader& mDownsampleShader = Util::g_shaders.bloom_downsample; 
static Shader& mUpsampleShader = Util::g_shaders.bloom_upsample;
static Shader& shaderBloomFinal = Util::g_shaders.bloom_final;
...
void FrameBuffer::RenderDownsamples(unsigned int srcTexture)
{
...
void FrameBuffer::RenderUpsamples(float filterRadius)
{
	const std::vector<bloomMip>& mipChain = mFBO.MipChain();
...
void FrameBuffer::RenderBloomTexture(float filterRadius)
{
	mFBO.BindForWriting();
...
GLuint FrameBuffer::getBloomTexture()
{
	return mFBO.MipChain()[0].texture;
}

GLuint FrameBuffer::BloomMip_i(int index)
{
	const std::vector<bloomMip>& mipChain = mFBO.MipChain();
